    ...
    <section className="relative overflow-hidden border-b border-white/6 py-20 sm:py-24">
      ...
      <Container className="relative">
        <div className="animate-rise">
          ...
          {description ? (
            <p className="mt-6 max-w-2xl text-base leading-relaxed text-mist/75">
              {description}
            </p>
          ) : null}
    ...
    <Card hover className="group flex flex-col overflow-hidden p-0">
      ...
      <div className="relative aspect-4/5 w-full overflow-hidden bg-gradient-to-br from-ink-600 to-ink">
        {leader.imageUrl ? (
          ...
          <img
            src={leader.imageUrl}
            alt={`${leader.username} - Clash of Editors leader`}
            loading="lazy"
            decoding="async"
        ...
        {/* Bottom gradient */}
        <div className="pointer-events-none absolute inset-x-0 bottom-0 h-32 bg-gradient-to-t from-ink via-ink/50 to-transparent" />
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <p className="mt-4 flex-1 text-[13px] leading-relaxed text-mist/65">
          {leader.biography ||
            "Biography will be published by the organizing team."}
        </p>
  ...
  return (
    <div className="flex flex-col rounded-2xl border border-dashed border-white/12 bg-white/[0.015] p-6">
      ...
      <p className="mt-4 text-[13px] leading-relaxed text-mist/55">
        This leader has not been announced yet. Details are published by the
        organizing team.
      </p>
    </div>
  );
  ...
  return (
    <ol className="relative space-y-4 border-l border-white/10 pl-6 sm:pl-8">
      {events.map((event) => {
        ...
        const isDerived =
          event.phaseKey === "notification_deadline" &&
          !event.eventDate &&
          computedDate;
        ...
        return (
          <li key={event.id} className="relative">
            ...
            <div className="rounded-2xl panel p-5 transition hover:border-gold/25 sm:p-6">
              ...
              {event.description ? (
                <p className="mt-3 text-[13px] leading-relaxed text-mist/65">
                  {event.description}
                </p>
              ) : null}
              ...
              {isDerived ? (
                <p className="mt-3 text-[11px] uppercase tracking-[0.16em] text-mist/45">
                  Auto-calculated: registration close +{" "}
                  {notificationWindow?.windowDays} days
                </p>
              ) : null}
...
export function NotificationWindowCard({
  window,
}: {
        ...
        <div>
          ...
          <dd className="mt-1 font-display text-lg font-bold text-white">
            {window.daysRemaining === null
              ? "TBA"
              : Math.max(window.daysRemaining, 0)}
          </dd>
        </div>
      </dl>

      <p className="mt-5 text-[13px] leading-relaxed text-mist/65">
        {window.detail}
      </p>
    </Card>
  );
